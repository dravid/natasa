...
switch ($page) {
	...
	case 'pages':
		...
		if(isset($_POST['submitted']) == 1){
			...
			if($r){
				...
				$message = '<p class="alert alert-danger">Page could not be '.$action.' becouse: '.mysqli_error($dbc).'</p>';
	...
	case 'users':
		...
		if(isset($_POST['submitted']) == 1){
			...
			if($r){
				...
				$message = '<p class="alert alert-danger">User could not be '.$action.' becouse: '.mysqli_error($dbc).'</p>';
	...
	case 'navigation':
		...
		if(isset($_POST['submitted']) == 1){
			...
			if($r){
				...
				$message = '<p class="alert alert-danger">Navigation Item could not be '.$action.' becouse: '.mysqli_error($dbc).'</p>';
				$message .= '<p class="alert alert-warning">Query:'.$q.'</p>';
			}
		}
	...
	case 'settings':
		...
		if(isset($_POST['submitted']) == 1){
			...
			if($r){
				$message = '<p class="alert alert-success">Seting was '.$action.'!</p>';
			} else {
				$message = '<p class="alert alert-danger">Setting could not be '.$action.' becouse: '.mysqli_error($dbc).'</p>';
				$message .= '<p class="alert alert-warning">Query:'.$q.'</p>';
			}
		}
